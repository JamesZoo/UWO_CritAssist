            ...
            while (this.count == currentCount)
            {
                switch (countingState)
                {
                    case CountingState.Activate:
                        if (firstMelee)
                        {
                            announcer.AnnounceAsync("Activating first melee.", 10, 150, true);
                            countingState = CountingState.Four;
                        }
                        else
                        {
                            announcer.AnnounceAsync("Activating normal melee.", 10, 150, true);
                    ...
                    case CountingState.Seven:
                        announcer.AnnounceAsync("Seven", -5, 200);
                        countingState = CountingState.Six;
                        await Task.Delay(1000);
                        break;

                    case CountingState.Six:
                        announcer.AnnounceAsync("Six", -5, 200);
                        countingState = CountingState.Five;
                        await Task.Delay(1000);
                        break;
                    ...
                    case CountingState.Defense:
                        announcer.AnnounceAsync("Defense", 10, 200);
                        countingState = CountingState.Six;
                        await Task.Delay(1000);
                        break;
