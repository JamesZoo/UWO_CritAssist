...
| `FinalAnalysisView.swift` | Renders the journey summary + final markdown document. Auto-runs `FinalAnalysisViewModel.run()` on appear via `.task` (no button tap needed for the first analysis). Toolbar shows Close + Re-analyze (re-runs when already analyzed) + a `ShareLink` share icon button. The share button opens the system share sheet directly (one tap, no intermediate sheet). The shared item is `RecipeAnalysisExport` (defined in the same file), a `Transferable` type that provides two representations: PDF data (via `UIGraphicsPDFRenderer` + Core Text framesetter, multi-page US Letter) for file-aware targets (Files, AirDrop, Mail, Print), and plain text (via `ProxyRepresentation`) for text-aware targets (Notes, iMessage, WeChat, etc.). iOS picks the right representation per target automatically. |
