...
| `FinalAnalysisView.swift` | Renders the journey summary + final markdown document. Auto-runs `FinalAnalysisViewModel.run()` on appear via `.task` (no button tap needed for the first analysis). Toolbar shows Close + Re-analyze (re-runs when already analyzed) + a share icon button. The share button opens a `ShareOptionsSheet` (private, in the same file) that offers two `ShareLink` rows: "Share as formatted text" (Markdown string → system share sheet, works with Notes, iMessage, WeChat, etc.) and "Save / share as PDF" (generates a multi-page US-Letter PDF via `UIGraphicsPDFRenderer` + Core Text framesetter on a background task, then offers it as a file to the system share sheet). |
