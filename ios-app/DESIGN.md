...
| `FinalAnalysisView.swift` | Two-state view: (1) **Serving picker** — shown before the first analysis; large `+`/`−` stepper (range 1–24, default `recipe.servings ?? 4`) with a live scale-factor hint, and an "Analyze" button that triggers the run; (2) **Result** — journey summary card (with inline `Stepper` for `vm.targetServings` and an orange hint when the count has been changed but not yet re-analyzed) + final document card. Toolbar: Close · Share icon · Re-analyze. The share item is `RecipeAnalysisExport` (same file), a `Transferable` type with PDF (`UIGraphicsPDFRenderer` + Core Text framesetter, multi-page US Letter) and text (`ProxyRepresentation`) representations — one tap, system share sheet. |
